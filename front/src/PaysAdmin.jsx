...
export default function PaysAdmin() {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [modalType, setModalType] = useState(null)
  const [selectedPays, setSelectedPays] = useState(null)
  const [newPaysName, setNewPaysName] = useState('')
  const [searchTerm, setSearchTerm] = useState('')
  const [paysList, setPaysList] = useState([
    { id: 1, name: 'France' },
    { id: 2, name: 'États-Unis' },
    { id: 3, name: 'Brésil' },
    { id: 4, name: 'Chine' },
  ...
  const openChoiceModal = (pays) => {
    setSelectedPays(pays)
    setModalType('choice')
  }

  const openEditModal = (pays) => {
    setSelectedPays(pays)
    setNewPaysName(pays.name)
    setModalType('edit')
  }

  const openDeleteModal = (pays) => {
    setSelectedPays(pays)
    setModalType('delete')
  }
  ...
  const handleAddPays = () => {
    if (newPaysName.trim() === '') return
    const id = Date.now()
    setPaysList([...paysList, { id, name: newPaysName.trim() }])
    closeModal()
  }

  const handleSaveEdit = () => {
    if (newPaysName.trim() === '') return
    setPaysList(paysList.map(p =>
      p.id === selectedPays.id ? { ...p, name: newPaysName.trim() } : p
    ))
    closeModal()
  }

  const handleConfirmDelete = () => {
    setPaysList(paysList.filter(p => p.id !== selectedPays.id))
    closeModal()
  }
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        <header className="flex items-center justify-between bg-white p-4 shadow-md md:hidden">
          ...
          <button onClick={() => setSidebarOpen(!sidebarOpen)}>
            {sidebarOpen ? (
              <svg className="w-8 h-8" viewBox="0 0 24 24"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2" /></svg>
            ) : (
      ...
      {modalType && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-80 space-y-4">
            {modalType === 'choice' && (
              <>
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={() => openEditModal(selectedPays)} className="bg-blue-600 text-white py-2 rounded hover:bg-blue-700">✏️ Modifier</button>
                  <button onClick={() => openDeleteModal(selectedPays)} className="bg-red-600 text-white py-2 rounded hover:bg-red-700">🗑️ Supprimer</button>
            ...
            {modalType === 'edit' && (
              <>
                <h2 className="text-xl font-bold">Modifier le pays</h2>
                <input
                  type="text"
                  value={newPaysName}
                  onChange={(e) => setNewPaysName(e.target.value)}
            ...
            {modalType === 'add' && (
              <>
                <h2 className="text-xl font-bold">Ajouter un pays</h2>
                <input
                  type="text"
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={closeModal} className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Annuler</button>
                  <button onClick={handleAddPays} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Ajouter</button>
                </div>
              </>
            )}
          </div>
