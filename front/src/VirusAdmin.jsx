...
export default function VirusAdmin() {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [modalType, setModalType] = useState(null)
  const [selectedVirus, setSelectedVirus] = useState(null)
  const [newVirusName, setNewVirusName] = useState('')
  const [searchTerm, setSearchTerm] = useState('')
  const [virusList, setVirusList] = useState([
    { id: 1, name: 'Virus Ebola' },
    { id: 2, name: 'Virus Zika' },
    { id: 3, name: 'Virus de la grippe H1N1' },
    { id: 4, name: 'Coronavirus SARS-CoV-2' },
    { id: 5, name: 'VIH' },
  ])
  ...
  const openChoiceModal = (virus) => {
    setSelectedVirus(virus)
    setModalType('choice')
  }

  const openEditModal = (virus) => {
    setSelectedVirus(virus)
    setNewVirusName(virus.name)
    setModalType('edit')
  }

  const openDeleteModal = (virus) => {
    setSelectedVirus(virus)
    setModalType('delete')
  }
  ...
  const handleAddVirus = () => {
    if (newVirusName.trim() === '') return
    const id = Date.now()
    setVirusList([...virusList, { id, name: newVirusName.trim() }])
    closeModal()
  }

  const handleSaveEdit = () => {
    if (newVirusName.trim() === '') return
    setVirusList(virusList.map(v =>
      v.id === selectedVirus.id ? { ...v, name: newVirusName.trim() } : v
    ))
    closeModal()
  }

  const handleConfirmDelete = () => {
    setVirusList(virusList.filter(v => v.id !== selectedVirus.id))
    closeModal()
  }
  ...
  const filteredVirus = virusList.filter(v =>
    ...
    <div className="relative flex flex-col md:flex-row h-screen bg-gray-100 overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        <header className="flex items-center justify-between bg-white p-4 shadow-md md:hidden">
          <h1 className="text-xl font-bold">Gestion des Virus</h1>
        ...
        <main className="p-4 md:p-6 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <h1 className="text-3xl font-bold">Gestion des Virus</h1>
            <div className="flex flex-col md:flex-row gap-2 items-stretch md:items-center">
              <input
                type="text"
                placeholder="🔍 Rechercher un virus..."
                value={searchTerm}
                ...
                {filteredVirus.map(v => (
                  <tr key={v.id}>
                    ...
                    <td className="px-6 py-4 text-right space-x-4">
                      <button onClick={() => openEditModal(v)} className="text-blue-600 hover:text-blue-900">✏️ Modifier</button>
                      <button onClick={() => openDeleteModal(v)} className="text-red-600 hover:text-red-900">🗑️ Supprimer</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="block md:hidden space-y-4">
            {filteredVirus.map(v => (
              <div key={v.id} onClick={() => openChoiceModal(v)} className="cursor-pointer bg-white rounded-xl p-4 shadow hover:bg-gray-100">
                <div className="flex justify-between items-center">
                  <div className="font-bold text-lg">{v.name}</div>
                  <div className="text-sm text-gray-500">#{v.id}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {modalType && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-80 space-y-4">
            {modalType === 'choice' && (
              <>
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={() => openEditModal(selectedVirus)} className="bg-blue-600 text-white py-2 rounded hover:bg-blue-700">✏️ Modifier</button>
                  <button onClick={() => openDeleteModal(selectedVirus)} className="bg-red-600 text-white py-2 rounded hover:bg-red-700">🗑️ Supprimer</button>
            ...
            {modalType === 'edit' && (
              <>
                <h2 className="text-xl font-bold">Modifier le virus</h2>
                <input
                  type="text"
                  value={newVirusName}
                  onChange={(e) => setNewVirusName(e.target.value)}
            ...
            {modalType === 'add' && (
              <>
                <h2 className="text-xl font-bold">Ajouter un virus</h2>
                <input
                  type="text"
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={closeModal} className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Annuler</button>
                  <button onClick={handleAddVirus} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Ajouter</button>
                </div>
              </>
            )}
          </div>
