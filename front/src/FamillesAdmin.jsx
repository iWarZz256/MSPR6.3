...
export default function FamillesAdmin() {
  ...
  const [familles, setFamilles] = useState([
    { id: 1, name: 'Orthomyxoviridae' },
    { id: 2, name: 'Coronaviridae' },
    { id: 3, name: 'Flaviviridae' },
    { id: 4, name: 'Retroviridae' },
    { id: 5, name: 'Filoviridae' },
  ])
  ...
  const openChoiceModal = (famille) => {
    setSelectedFamille(famille)
    setModalType('choice')
  }

  const openEditModal = (famille) => {
    setSelectedFamille(famille)
    setNewFamilleName(famille.name)
    setModalType('edit')
  }

  const openDeleteModal = (famille) => {
    setSelectedFamille(famille)
    setModalType('delete')
  }
  ...
  const handleAddFamille = () => {
    ...
    const id = Date.now()
    setFamilles([...familles, { id, name: newFamilleName }])
    closeModal()
  }
  ...
  const handleSaveEdit = () => {
    ...
    setFamilles(familles.map(f =>
      f.id === selectedFamille.id ? { ...f, name: newFamilleName } : f
    ))
    closeModal()
  }

  const handleConfirmDelete = () => {
    setFamilles(familles.filter(f => f.id !== selectedFamille.id))
    closeModal()
  }
    ...
    <div className="relative flex flex-col md:flex-row h-screen bg-gray-100 overflow-x-hidden">
            ...
            <div className="ml-4 space-y-1">
              {cards.map(card => (
                <Link key={card.to} to={card.to}
                  className={`flex items-center gap-2 py-1 px-2 rounded-lg text-sm transition ${location.pathname === card.to ? 'bg-blue-700 font-bold' : 'hover:bg-blue-600'}`}>
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        <header className="flex items-center justify-between bg-white p-4 shadow-md md:hidden">
          <h1 className="text-xl font-bold">Gestion des Familles</h1>
          <button onClick={() => setSidebarOpen(!sidebarOpen)}>
            <svg className="w-8 h-8" viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h16" stroke="currentColor" strokeWidth="2" /></svg>
          </button>
        </header>

        <main className="p-4 md:p-6 space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <h1 className="text-3xl font-bold hidden md:block">Gestion des Familles de Virus</h1>
                ...
                {filteredFamilles.map(f => (
                  <tr key={f.id}>
                    ...
                    <td className="px-6 py-4 text-right space-x-4">
                      <button onClick={() => openEditModal(f)} className="text-blue-600 hover:text-blue-900">✏️ Modifier</button>
                      <button onClick={() => openDeleteModal(f)} className="text-red-600 hover:text-red-900">🗑️ Supprimer</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="block md:hidden space-y-4">
            {filteredFamilles.map(f => (
              <div key={f.id} onClick={() => openChoiceModal(f)} className="cursor-pointer bg-white rounded-xl p-4 shadow hover:bg-gray-100">
                <div className="flex justify-between items-center">
                  <div className="font-bold text-lg">{f.name}</div>
                  <div className="text-sm text-gray-500">#{f.id}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {modalType && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-80 space-y-4">
            {modalType === 'choice' && (
              <>
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={() => openEditModal(selectedFamille)} className="bg-blue-600 text-white py-2 rounded hover:bg-blue-700">✏️ Modifier</button>
                  <button onClick={() => openDeleteModal(selectedFamille)} className="bg-red-600 text-white py-2 rounded hover:bg-red-700">🗑️ Supprimer</button>
            ...
            {modalType === 'edit' && (
              <>
                <h2 className="text-xl font-bold">Modifier la famille</h2>
                <input
                  type="text"
            ...
            {modalType === 'add' && (
              <>
                <h2 className="text-xl font-bold">Ajouter une famille</h2>
                <input
                  type="text"
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={closeModal} className="bg-gray-300 px-4 py-2 rounded hover:bg-gray-400">Annuler</button>
                  <button onClick={handleAddFamille} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Ajouter</button>
                </div>
              </>
            )}
          </div>
